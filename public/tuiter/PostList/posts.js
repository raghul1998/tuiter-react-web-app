let posts = [
    {
        avatar: '../images/elonmuskavatar.jpg',
        userName: 'Elon Musk',
        handle: 'elonmusk',
        time: '23h',
        title: 'Amazing how about @inspiration4x mission!',
        image: '../images/inspiration4.jpg',
        imageTitle: 'Countdown: Inspiration4 Mission to Space | Netflix Official Site',
        imageDescription: 'From training to launch to landing, this all-access docuseries rides'
                          + ' along with the Inspiration4 crew on the first all-civilian orbital'
                          + ' space',
        imageLink: 'netflix.com',
        comments: '4.2K',
        retuits: '3.5K',
        likes: '35K'
    },
    {
        avatar: '../images/new_york_post.jpg',
        userName: 'New York Post',
        handle: 'nypost',
        time: '23h',
        title: 'Grimes seen reading Karl Marx following split with world\'s richest man Elon Musk',
        image: '../images/grimes.jpg',
        imageTitle: '',
        imageDescription: '',
        imageLink: '',
        comments: '965',
        retuits: '2.4K',
        likes: '4K'
    }
]

export default posts;